 ...
 */
import { assertAllowedFetchUrl, readBoundedText, DEFAULT_FETCH_PORTS } from "./source-safety.js";
import { checkRobots } from "./robots.js";
import { docIdFromUrl, contentHashOf } from "./etl-store.js";
...
function mainBodyFrom(text, snippetMaxChars) {
...
// ── Paywall detection ────────────────────────────────────────────────────────
function detectPaywall(html, domain) {
    if (TOS_DISALLOW_DOMAINS.has(domain))
        return true;
...
export async function fetchArticle(url, opts = {}) {
    ...
    try {
        if (crawlDelaySeconds && crawlDelaySeconds > 0) {
            await new Promise((r) => setTimeout(r, Math.min(crawlDelaySeconds * 1000, 5000)));
        }
        const headers = {
...
function makeFailedDoc(url, fetchedAt, accessPolicy, opts) {
...
function makeNotModifiedDoc(url, fetchedAt, opts) {
...
export function wordCountOf(body) {
    if (!body)
        return 0;
