...
function parseArgs() {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        else if (a === '--max-age-hours' && args[i + 1]) {
            maxAgeHours = Number(args[++i]) || 36;
        }
        else if (a === '--timeout' && args[i + 1]) {
            timeoutMs = Number(args[++i]) || 15_000;
        }
        else if (a === '--concurrency' && args[i + 1]) {
            concurrency = Number(args[++i]) || 10;
        }
        else if (a === '--etl') {
            etlEnabled = true;
        }
        else if (a === '--etl-budget' && args[i + 1]) {
            etlFetchBudgetPerTopic = Number(args[++i]) || 30;
        }
    }
    return { outPath, maxAgeHours, timeoutMs, concurrency, etlEnabled, etlFetchBudgetPerTopic };
}
async function main() {
    const { outPath, maxAgeHours, timeoutMs, concurrency, etlEnabled, etlFetchBudgetPerTopic } = parseArgs();
    const now = new Date();
    ...
    for (const id of topicIds) {
        ...
        process.stderr.write(`  [${id}] ${items.length} items · ${clusters.length} clusters · ` +
    ...
    if (etlEnabled) {
        const docs = artifact.data.documentsByTopic;
        const facts = artifact.data.factsByCluster;
