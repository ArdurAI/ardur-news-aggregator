...
function similarity(a, b, importantSet) {
...
export function clusterItems(items, options) {
    ...
    for (const item of sorted) {
        const found = rawClusters.find((c) => c.topic === item.topic && similarity(c.headline, item.title, importantSet) >= threshold);
        if (found) {
    ...
    for (const raw of rawClusters) {
        ...
        for (const m of raw.members) {
            ...
            if (m.publishedAt < earliest)
                earliest = m.publishedAt;
            if (m.publishedAt > latest)
