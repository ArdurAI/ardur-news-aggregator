...
import { runAggregation } from "./index.js";
import { SCHEMA_VERSION, CONTRACT_REVISION, } from '@ardurai/contracts';
export function buildDescribeOutput() {
    return {
...
// ---------------------------------------------------------------------------
export function deriveRunId(nowIso) {
...
export function buildHermeticArtifact(now, runId) {
...
async function main() {
    ...
    const resolvedRunId = args.runId ?? (args.nowIso ? deriveRunId(args.nowIso) : undefined);
    if (args.provider !== 'deterministic') {
        process.env['ARDUR_AI_PROVIDER'] = args.provider;
    }
    if (args.fixturesDir !== null) {
        process.stderr.write('[ardur-news-aggregator] --fixtures is reserved for future implementation; ' +
    ...
    try {
        ...
        if (args.jsonErrors) {
            const envelope = { error: classifyError(err) };
            process.stdout.write(JSON.stringify(envelope, null, 2) + '\n');
        }
        else {
            process.stderr.write(`[ardur-news-aggregator] fatal: ${err instanceof Error ? err.message : String(err)}\n`);
        }
        process.exitCode = 1;
        return;
    }
    ...
    if (args.outPath !== null) {
        ...
        process.stderr.write(`[ardur-news-aggregator] wrote ${json.length} bytes → ${args.outPath}\n`);
    }
    else {
...
main().catch((err) => {
    process.stderr.write(`[ardur-news-aggregator] unhandled: ${err instanceof Error ? err.message : String(err)}\n`);
    process.exitCode = 1;
});
