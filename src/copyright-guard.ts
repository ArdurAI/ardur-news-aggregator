 ...
 */
export function validateFactsForWire(facts: ExtractedFact[]): FactWireValidationResult {
  ...
  for (const fact of facts) {
    ...
    for (const prov of fact.provenance) {
      ...
      if (prov.quote !== undefined) {
        ...
        if (wc > MAX_QUOTE_WORDS) {
          violations.push(
            `fact ${fact.id}: quote is ${wc} words in provenance from ${prov.sourceDomain} — truncated`,
          );
